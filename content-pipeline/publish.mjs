...
Generated by content-pipeline. Editorial pass complete; all gates passed.`;
  ...
  // Author identity: pass via -c so we don't depend on `git config` being set
  // on the deploy host. Falls back to env vars if explicitly provided.
  const gitName = process.env.PIPELINE_GIT_NAME || 'SwiftMail Pipeline';
  const gitEmail = process.env.PIPELINE_GIT_EMAIL || 'pipeline@swift-mail.app';
  const ident = `-c user.name=${JSON.stringify(gitName)} -c user.email=${JSON.stringify(gitEmail)}`;
  try {
    execSync(`git add -A`, { cwd: REPO_ROOT, stdio: 'inherit' });
    execSync(`git ${ident} commit -m ${JSON.stringify(message)}`, { cwd: REPO_ROOT, stdio: 'inherit' });
